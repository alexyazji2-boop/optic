...
def size_shares(entry: float, stop: float, equity: float) -> Dict[str, Any]:
    ...
    notional = qty * entry
    # Cap notional at 25% of equity: sizing purely off a very tight stop can
    # otherwise produce a position larger than the account.
...
def size_option(premium: float, equity: float) -> Dict[str, Any]:
    ...
    budget = equity * min(RISK_PER_TRADE * 2, MAX_OPTION_RISK)
...
def consider_ticker(snapshot: Dict[str, Any],
                    equity: float) -> tuple[List[Dict[str, Any]], List[str]]:
    ...
    share_size = size_shares(spot, stop, equity)
    if share_size["qty"] <= 0:
    ...
    if not pick.get("strike") or not pick.get("expiry") or not pick.get("entry_mid"):
        ...
        opt_size = size_option(float(pick["entry_mid"]), equity)
        if opt_size["qty"] <= 0:
...
def _capacity(equity: float) -> Dict[str, Any]:
    ...
    """
    rows = _rows("SELECT id FROM positions WHERE status='open'")
    risk = open_risk()
    return {
        "open_positions": len(rows),
        "position_slots_left": max(MAX_OPEN_POSITIONS - len(rows), 0),
        "open_risk": _f(risk, 2),
        "open_risk_pct": _f(risk / equity * 100.0, 2) if equity else None,
        "risk_budget_left": _f(max(equity * MAX_PORTFOLIO_RISK - risk, 0.0), 2),
    }
...
def run_scan(snapshot_fn: Callable[[str], Dict[str, Any]], provider,
    ...
    try:
        ...
        for index, ticker in enumerate(candidates_in):
            ...
            found, why = consider_ticker(snapshot, equity)
            notes.extend(why)
            for candidate in found:
                # Check the budget against what this position would *add*, not just
                # what's already deployed. Testing only the running total let a
                # ticker that started inside the budget push the book past it —
                # the panel then read "17.6% of a 15% budget", which is the cap
                # describing itself being broken.
                room = _capacity(equity)["risk_budget_left"] or 0.0
                cost = candidate.get("risk_dollars") or 0.0
                if cost > room:
                    notes.append("{}: {} leg not taken — its {} of risk doesn't fit the {} left "
                                 "in the portfolio budget".format(
                                     ticker, candidate["instrument"],
                                     _money(cost), _money(room)))
                    continue
        ...
        with _LOCK, _connect() as conn:
            conn.execute(
                ...
                " VALUES (?,?,?,?,?,?,?)",
                (started, trigger, considered, opened, marks["closed"],
                 json.dumps(notes[:60]), json.dumps(funnel)),
            )
        return {"considered": considered, "opened": opened, "closed": marks["closed"],
...
def summary() -> Dict[str, Any]:
    ...
    closed = _rows("SELECT * FROM positions WHERE status='closed' ORDER BY exit_at DESC")
    if LEDGER_START:
        closed = [r for r in closed
                  if (_month_key(r.get("exit_at")) or LEDGER_START) >= LEDGER_START]
    open_rows = _rows("SELECT * FROM positions WHERE status='open' ORDER BY entry_at DESC")
...
def state(limit: int = 60, month: Optional[str] = None) -> Dict[str, Any]:
    init_db()
    ...
    return {
        ...
        "capacity": _capacity(summary()["equity"]),
        "open": _rows("SELECT * FROM positions WHERE status='open' ORDER BY entry_at DESC"),
        "closed": _rows(
            "SELECT * FROM positions WHERE status='closed' ORDER BY exit_at DESC LIMIT ?",
            (limit,),
        ),
        "scans": scans,
        "config": {
